...
class JobMatch(db.Model):
    ...
    roadmap_result = db.Column(db.Text, nullable=True)  # Cached JSON from preparation roadmap
    ...
    def to_dict(self):
        return {
            ...
            'roadmap_result': json.loads(self.roadmap_result) if self.roadmap_result else None
        }
...
class TaskPlan(db.Model):
    """A long-horizon plan generated by the AI planner for a user's career goal."""
    ...
    user = db.relationship('User', backref=db.backref('task_plans', lazy='dynamic', cascade='all, delete-orphan'))
    steps = db.relationship('PlanStep', backref='plan', lazy='dynamic',
...
class PlanStep(db.Model):
    """A single step within a TaskPlan, generated and managed by the AI."""
    __tablename__ = 'plan_steps'

    id = db.Column(db.Integer, primary_key=True)
    plan_id = db.Column(db.Integer, db.ForeignKey('task_plans.id'), nullable=False, index=True)
    step_order = db.Column(db.Integer, nullable=False)
    description = db.Column(db.Text, nullable=False)
    tool_name = db.Column(db.String(100), nullable=True)  # which tool to call, or null for LLM-only
    tool_input = db.Column(db.Text, nullable=True)  # JSON string of tool arguments
    status = db.Column(db.String(20), default='pending')  # 'pending', 'running', 'done', 'skipped'
    result_summary = db.Column(db.Text, nullable=True)  # what the step produced
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
